...
import edu.umass.cs.mallet.grmm.learning.ACRF.MaximizableACRF;
...
//		Maximizer.ByGradient maximizer = new GradientDescent ();
  ...
  public static class LogEvaluator extends ACRFEvaluator {
    ...
    public LogEvaluator(String filename) {
		...
		try {
			new PrintWriter (new FileWriter (filename + "Training" + ".txt", false));
			new PrintWriter (new FileWriter (filename + "Testing" + ".txt", false));
		} catch (IOException e) {
    ...
    public void test (InstanceList testList, List returnedList,
                      String description)
    {
      String logFilename = filename + description + ".txt";
...
//			System.out.println ("\n\nInstance");
        ...
        for (int i = 0; i < rows; i++) {
        	writer.println(target.get(i).toString() + " " + returned.get(i).toString());
        }
...
//			System.out.println ("Time "+i);
  ...
  public static class FileEvaluator extends ACRFEvaluator {
                      ...
                      String description)
    {
      ...
      TestResults results = LogEvaluator.computeTestResults (testList, returnedList, "somefile_");
...
//		results.printConfusion ();
  ...
  public static class TestResults {
    ...
    {
      String tab = "\t\t";
      logger.info (desc+":  i" + tab + "Label" + tab + "N" + tab + "Correct" + 
    		  tab + "Returned" + tab + "P" + tab + "R" + tab + "F1");
      for (int i = 0; i < numClasses; i++) {
        logger.info (desc+":  "+i + tab + alphabet.lookupObject (i) + tab
                + trueCounts[i] + tab
                + confusion[i][i] + tab
                + returnedCounts[i] + tab
                + precision[i] + tab
                + recall[i] + tab
                + f1[i] + tab);
      }
